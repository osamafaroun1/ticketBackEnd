...
ticketRouter.get('/tickets', requireAuth, requireRole(['CIVIL', 'ADMIN', 'SUPERADMIN']), async (req, res) => {
  ...
  // Optional query filters (for ADMIN and SUPERADMIN)
  if (req.query.statusId) {
    where.push('t.status_id = :statusId');
    params.statusId = Number(req.query.statusId);
  }
  if (req.query.provinceId && user.roleName === 'SUPERADMIN') {
    where.push('t.province_id = :filterProvinceId');
    params.filterProvinceId = Number(req.query.provinceId);
  }
  if (req.query.departmentId) {
    where.push('t.department_id = :filterDepartmentId');
    params.filterDepartmentId = Number(req.query.departmentId);
  }
  if (req.query.issueTypeId) {
    where.push('t.issue_type_id = :filterIssueTypeId');
    params.filterIssueTypeId = Number(req.query.issueTypeId);
  }
  if (req.query.dateFrom) {
    where.push('DATE(t.created_at) >= :dateFrom');
    params.dateFrom = req.query.dateFrom;
  }
  if (req.query.dateTo) {
    where.push('DATE(t.created_at) <= :dateTo');
...
ticketRouter.patch('/tickets/:id/status', requireAuth, requireRole(['ADMIN', 'SUPERADMIN']), async (req, res) => {
  ...
  await pool.query(
    ...
    { ticketId: id, fromStatusId, toStatusId, changedBy: user.userId, note: parsed.data.note ?? 'Updated' }
  );
